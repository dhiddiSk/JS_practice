...
//delete the event of yellow card
const gameEventsAfterDeletingEvent = gameEvents.delete(64);
console.log(gameEvents);

//The number of events happened in average time.
console.log(`An event happened, on average, every ${([...gameEvents.keys()].pop())/gameEvents.size} minutes`);

//Print the event to the console referring to the first half or second half.
const printEventType = function(events){
	for(const [min, event] of events){
		let type = (min <= 45) ? "First Half" : "Second Half";
		console.log(`${type} ${min} : ${event}`);
	}
};
console.log(printEventType(gameEvents));
